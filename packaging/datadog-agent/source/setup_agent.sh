...
print_console "* Downloading integrations from GitHub (~5 MB)"
mkdir -p "$DD_HOME/3rd-party"
$DOWNLOADER "$DD_HOME/integrations.tar.gz" "https://api.github.com/repos/DataDog/integrations-core/tarball"
print_done

print_console "* Uncompressing tarball"
tar -xz -C "$DD_HOME/3rd-party" --strip-components 1 -f "$DD_HOME/integrations.tar.gz"
rm -f "$DD_HOME/integrations.tar.gz"
print_done

print_console "* Trying to install integration requirements"
for INT_DIR in "$DD_HOME/3rd-party/*"; do
  if [ -d $INT_DIR ]; then
